...
const DonorEssentialDonate = () => {
    ...
    // Function to find the corresponding image for an item
    const findImage = (itemName) => {
        const item = dbItems.find((item) => item.ItemName === itemName);
        ...
        <>
            <div className="mt-12 md:mt-24">
                ...
                <div className="flex justify-center mb-4">
                    ...
                    />
                    <select onChange={handleMonthFilter} value={selectedMonth} className="px-4 py-2 border border-gray-300 rounded-md">
                ...
                {currentItems.filter((val) => {
                    ...
                    <div key={index} className="bg-white rounded-xl shadow-lg">
                        <div className="overflow-hidden rounded-t-xl">
                            {findImage(item.ItemName) && (
                                <img src={findImage(item.ItemName)} alt={item.ItemName} className="w-full h-48 object-cover object-center" />
                            )}
                        </div>
                        <div className="p-6">
            ...
            <div className="flex justify-center mt-8">
                <button onClick={() => handlePagination("prev")} className="px-4 py-2 mr-2 bg-black text-white rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600 cursor-pointer" disabled={currentPage === 1}>
                    Previous
                </button>
                <button onClick={() => handlePagination("next")} className="px-4 py-2 bg-black text-white rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600" disabled={currentPage === totalPages}>
                    Next
                </button>
            </div>
        </>
