...
const CriticalMedication = () => {
	...
	const filteredMedications = criticalMedications
		.filter((medication) => {
							...
							{currentMedications.map((medication, index) => (
								<tr key={index} className={index % 2 === 0 ? "bg-gray-100" : "bg-white"}>
									...
									<td className="px-4 py-2">
										<div
											className={`text-center px-4 py-2 ${medication.Priority === 1 ? "bg-red-300 ml-5 w-32 border-2 border-red-600 text-red-900" : "bg-green-300 w-44 border-2 text-green-900 border-green-600"} py-1 rounded shadow-lg`}
										>
											{medication.Priority === 1.0 ? "High" : "Low"}
										</div>
									</td>
								</tr>
							))}
